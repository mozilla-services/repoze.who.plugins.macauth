...
from repoze.who.plugins.macauth.noncemanager import NonceManager, Cache


class TestNonces(unittest2.TestCase):

    def test_default_timeout_is_one_minute(self):
        nm = NonceManager()
        self.assertEquals(nm.nonce_timeout,  60)
        self.assertEquals(nm.token_timeout,  60)

    def test_operation(self):
        timeout = 0.1
        nm = NonceManager(nonce_timeout=timeout, token_timeout=1)
        # Initially nothing is cached, so all nonces as fresh.
        self.assertEquals(nm.nonce_timeout, 0.1)
        self.assertEquals(len(nm), 0)
        self.assertTrue(nm.is_fresh("token", time.time(), "abc"))
        # After adding a nonce, it should contain just that item.
        nm.add_nonce("token", time.time(), "abc")
        self.assertEquals(len(nm), 1)
        self.assertFalse(nm.is_fresh("token", time.time(), "abc"))
        self.assertTrue(nm.is_fresh("token", time.time(), "xyz"))
        ...
        nm.add_nonce("token", time.time(), "def")
        self.assertFalse(nm.is_fresh("token", time.time(), "abc"))
        self.assertFalse(nm.is_fresh("token", time.time(), "def"))
        self.assertTrue(nm.is_fresh("token", time.time(), "xyz"))
        time.sleep(timeout / 2)
        nm.add_nonce("token", time.time(), "xyz")
        self.assertTrue(nm.is_fresh("token", time.time(), "abc"))
        self.assertFalse(nm.is_fresh("token", time.time(), "def"))
        self.assertFalse(nm.is_fresh("token", time.time(), "xyz"))
